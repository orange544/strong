param(
  ...
  [string]$SchemaUser = 'GAOXIAO_DB11',
  [string]$SchemaPassword = 'GaoxiaoDb11_123'
)
...
$schemaFile = Join-Path $PSScriptRoot 'db11_oracle.sql'
if (-not (Test-Path $schemaFile)) {
  throw 'DB11 Oracle schema file not found.'
}
...
$bootstrapSql = Join-Path $env:TEMP ("db11_bootstrap_" + [guid]::NewGuid().ToString('N') + '.sql')
$verifySql = Join-Path $env:TEMP ("db11_verify_" + [guid]::NewGuid().ToString('N') + '.sql')
...
SELECT COUNT(*) FROM dorm_building;
SELECT COUNT(*) FROM dorm_room;
SELECT COUNT(*) FROM bed_allocation;
SELECT COUNT(*) FROM checkin_record;
SELECT COUNT(*) FROM dorm_transfer;
SELECT COUNT(*) FROM late_return_record;
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw 'DB11 Oracle bootstrap failed.'
  }

  & $sqlplus -L -S $userConnection "@$schemaFile"
  if ($LASTEXITCODE -ne 0) {
    throw 'DB11 Oracle schema/sample initialization failed.'
  }

  $verifyOutput = & $sqlplus -L -S $userConnection "@$verifySql"
  if ($LASTEXITCODE -ne 0) {
    throw 'DB11 Oracle verification query failed.'
  }
  ...
  if ($parsed.Count -lt 7) {
    throw 'DB11 Oracle verification result is incomplete.'
  }
  foreach ($v in $parsed) {
    if ($v -lt 1) {
      throw 'DB11 Oracle has table without sample data.'
    }
  }
}
finally {
...
Write-Host "DB11 Oracle schema and sample data initialized: $SchemaUser"
