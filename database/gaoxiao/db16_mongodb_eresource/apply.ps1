...
$schemaFile = Join-Path $PSScriptRoot 'db16_mongodb.js'
$sampleFile = Join-Path $PSScriptRoot 'db16_mongodb_sample_data.js'
if (-not (Test-Path $schemaFile)) {
  throw 'DB16 MongoDB schema file not found.'
}
if (-not (Test-Path $sampleFile)) {
  throw 'DB16 MongoDB sample data file not found.'
}
...
Write-Host 'DB16 MongoDB schema and sample data initialized: gaoxiao_db16_eresource'
