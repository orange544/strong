param(
  ...
  [string]$Database = 'gaoxiao_db21_research'
)
...
$schemaFile = Join-Path $PSScriptRoot 'db21_postgresql.sql'
$sampleFile = Join-Path $PSScriptRoot 'db21_postgresql_sample_data.sql'
if (-not (Test-Path $schemaFile)) {
  throw 'DB21 PostgreSQL schema file not found.'
}
if (-not (Test-Path $sampleFile)) {
  throw 'DB21 PostgreSQL sample data file not found.'
}
...
try {
  ...
  if (($exists | Out-String).Trim() -ne '1') {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw 'Failed to create DB21 PostgreSQL database.'
    }
  }

  & $psql -h $DbHost -p $Port -U $User -d $Database -v ON_ERROR_STOP=1 -f $schemaFile
  if ($LASTEXITCODE -ne 0) {
    throw 'Failed to apply DB21 PostgreSQL schema.'
  }

  & $psql -h $DbHost -p $Port -U $User -d $Database -v ON_ERROR_STOP=1 -f $sampleFile
  if ($LASTEXITCODE -ne 0) {
    throw 'Failed to apply DB21 PostgreSQL sample data.'
  }
  ...
  foreach ($table in $tables) {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "DB21 PostgreSQL verification query failed for table: $table"
    }

    $countValue = 0
    $raw = ($countText | Out-String).Trim()
    if (-not [int]::TryParse($raw, [ref]$countValue)) {
      throw "DB21 PostgreSQL verification parse failed for table: $table"
    }
    if ($countValue -lt 1) {
      throw "DB21 PostgreSQL table has no sample data: $table"
    }
  }
}
finally {
...
Write-Host "DB21 PostgreSQL schema and sample data initialized: $Database"
