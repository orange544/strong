...
$schemaFile = Join-Path $PSScriptRoot 'db22_mongodb.js'
$sampleFile = Join-Path $PSScriptRoot 'db22_mongodb_sample_data.js'
if (-not (Test-Path $schemaFile)) {
  throw 'DB22 MongoDB schema file not found.'
}
if (-not (Test-Path $sampleFile)) {
  throw 'DB22 MongoDB sample data file not found.'
}
...
Write-Host 'DB22 MongoDB schema and sample data initialized: gaoxiao_db22_achievement'
