param(
  ...
  [string]$SchemaUser = 'GAOXIAO_DB23',
  [string]$SchemaPassword = 'GaoxiaoDb23_123'
)
...
$schemaFile = Join-Path $PSScriptRoot 'db23_oracle.sql'
if (-not (Test-Path $schemaFile)) {
  throw 'DB23 Oracle schema file not found.'
}
...
$bootstrapSql = Join-Path $env:TEMP ("db23_bootstrap_" + [guid]::NewGuid().ToString('N') + '.sql')
$verifySql = Join-Path $env:TEMP ("db23_verify_" + [guid]::NewGuid().ToString('N') + '.sql')
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw 'DB23 Oracle bootstrap failed.'
  }

  & $sqlplus -L -S $userConnection "@$schemaFile"
  if ($LASTEXITCODE -ne 0) {
    throw 'DB23 Oracle schema/sample initialization failed.'
  }

  $verifyOutput = & $sqlplus -L -S $userConnection "@$verifySql"
  if ($LASTEXITCODE -ne 0) {
    throw 'DB23 Oracle verification query failed.'
  }
  ...
  if ($parsed.Count -lt 8) {
    throw 'DB23 Oracle verification result is incomplete.'
  }
  foreach ($v in $parsed) {
    if ($v -lt 1) {
      throw 'DB23 Oracle has table without sample data.'
    }
  }
}
finally {
...
Write-Host "DB23 Oracle schema and sample data initialized: $SchemaUser"
