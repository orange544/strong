param(
  ...
  [string]$Database = 'gaoxiao_db24_asset'
)
...
$schemaFile = Join-Path $PSScriptRoot 'db24_postgresql.sql'
$sampleFile = Join-Path $PSScriptRoot 'db24_postgresql_sample_data.sql'
if (-not (Test-Path $schemaFile)) {
  throw 'DB24 PostgreSQL schema file not found.'
}
if (-not (Test-Path $sampleFile)) {
  throw 'DB24 PostgreSQL sample data file not found.'
}
...
try {
  ...
  if (($exists | Out-String).Trim() -ne '1') {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw 'Failed to create DB24 PostgreSQL database.'
    }
  }

  & $psql -h $DbHost -p $Port -U $User -d $Database -v ON_ERROR_STOP=1 -f $schemaFile
  if ($LASTEXITCODE -ne 0) {
    throw 'Failed to apply DB24 PostgreSQL schema.'
  }

  & $psql -h $DbHost -p $Port -U $User -d $Database -v ON_ERROR_STOP=1 -f $sampleFile
  if ($LASTEXITCODE -ne 0) {
    throw 'Failed to apply DB24 PostgreSQL sample data.'
  }
  ...
  foreach ($table in $tables) {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "DB24 PostgreSQL verification query failed for table: $table"
    }

    $countValue = 0
    $raw = ($countText | Out-String).Trim()
    if (-not [int]::TryParse($raw, [ref]$countValue)) {
      throw "DB24 PostgreSQL verification parse failed for table: $table"
    }
    if ($countValue -lt 1) {
      throw "DB24 PostgreSQL table has no sample data: $table"
    }
  }
}
finally {
...
Write-Host "DB24 PostgreSQL schema and sample data initialized: $Database"
