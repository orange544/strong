...
$schemaFile = Join-Path $PSScriptRoot 'db2_mongodb.js'
$sampleFile = Join-Path $PSScriptRoot 'db2_mongodb_sample_data.js'
if (-not (Test-Path $schemaFile)) {
  throw 'DB2 MongoDB schema file not found.'
}
if (-not (Test-Path $sampleFile)) {
  throw 'DB2 MongoDB sample data file not found.'
}
...
Write-Host 'DB2 MongoDB schema and sample data initialized: gaoxiao_db2_material'
