param(
  ...
  [string]$Database = 'gaoxiao_db6_graduate'
)
...
$schemaFile = Join-Path $PSScriptRoot 'db6_postgresql.sql'
$sampleFile = Join-Path $PSScriptRoot 'db6_postgresql_sample_data.sql'
if (-not (Test-Path $schemaFile)) {
  throw 'DB6 PostgreSQL schema file not found.'
}
if (-not (Test-Path $sampleFile)) {
  throw 'DB6 PostgreSQL sample data file not found.'
}
...
try {
  ...
  foreach ($table in $tables) {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "DB6 PostgreSQL verification query failed for table: $table"
    }

    $countValue = 0
    $raw = ($countText | Out-String).Trim()
    if (-not [int]::TryParse($raw, [ref]$countValue)) {
      throw "DB6 PostgreSQL verification parse failed for table: $table"
    }
    if ($countValue -lt 1) {
      throw "DB6 PostgreSQL table has no sample data: $table"
    }
  }
}
finally {
...
Write-Host "DB6 PostgreSQL schema and sample data initialized: $Database"
