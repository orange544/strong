...
$schemaFile = Join-Path $PSScriptRoot 'school_b_mongodb.js'
$sampleFile = Join-Path $PSScriptRoot 'school_b_mongodb_sample_data.js'
if (-not (Test-Path $schemaFile)) {
  throw 'School B MongoDB schema file not found.'
}
if (-not (Test-Path $sampleFile)) {
  throw 'School B MongoDB sample data file not found.'
}
...
function Invoke-MongoScript([string]$ScriptFile) {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw "School B MongoDB script execution failed: $ScriptFile"
  }
}
...
$verifyJs = "const dbRef = db.getSiblingDB('university_document_db'); print(dbRef.teaching_materials.countDocuments({_id:'TM_CUR3001_2025FALL'}) + dbRef.teaching_materials.countDocuments({_id:'TM_CUR3002_2025FALL'})); print(dbRef.student_growth_archive.countDocuments({_id:'SGA_L20210088'}) + dbRef.student_growth_archive.countDocuments({_id:'SGA_L20210089'}));"
...
if ($LASTEXITCODE -ne 0) {
  throw 'School B MongoDB verification failed.'
}
...
if ($nums.Count -lt 2 -or [int]$nums[0] -lt 2 -or [int]$nums[1] -lt 2) {
  throw 'School B MongoDB collection count verification failed.'
}

Write-Host 'School B MongoDB database initialized: university_document_db'
