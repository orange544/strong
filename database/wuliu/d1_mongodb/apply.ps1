...
$schemaFile = Join-Path $PSScriptRoot 'd1_mongodb.js'
$sampleFile = Join-Path $PSScriptRoot 'd1_mongodb_sample_data.js'
if (-not (Test-Path $schemaFile)) {
  throw 'D1 MongoDB schema file not found.'
}
if (-not (Test-Path $sampleFile)) {
  throw 'D1 MongoDB sample data file not found.'
}
...
Write-Host 'D1 MongoDB schema and sample data initialized: wuliu_d1_mongodb_db'
