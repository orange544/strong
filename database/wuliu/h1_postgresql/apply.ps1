param(
  ...
  [string]$Database = 'wuliu_h1_postgresql_db'
)
...
$schemaFile = Join-Path $PSScriptRoot 'h1_postgresql.sql'
$sampleFile = Join-Path $PSScriptRoot 'h1_postgresql_sample_data.sql'
if (-not (Test-Path $sampleFile)) {
  throw 'H1 PostgreSQL sample data file not found.'
}
...
try {
  ...
  if ($LASTEXITCODE -ne 0) {
    throw 'Failed to apply H1 PostgreSQL schema.'
  }

  & $psql -h $DbHost -p $Port -U $User -d $Database -v ON_ERROR_STOP=1 -f $sampleFile
  if ($LASTEXITCODE -ne 0) {
    throw 'Failed to apply H1 PostgreSQL sample data.'
  }
}
finally {
...
Write-Host "H1 PostgreSQL schema and sample data initialized: $Database"
