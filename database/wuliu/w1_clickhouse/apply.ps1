...
$sqlFile = Join-Path $PSScriptRoot 'w1_clickhouse.sql'
if (-not (Test-Path $sqlFile)) {
  throw 'W1 ClickHouse SQL file not found.'
}
...
if ($code -ne 0) {
  throw 'W1 ClickHouse initialization failed.'
}

Write-Host 'W1 ClickHouse schema and sample data initialized: wuliu_w1_clickhouse_db'
