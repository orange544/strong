...
$rawLogPath = Join-Path $logDir "sample_raw_$timestamp.log"
$safeLogPath = Join-Path $logDir "sample_$timestamp.log"
$exitCode = 1

Push-Location $repoRoot
try {
    $args = @("-u", "main.py", "--mode", "sample")
    if ($UploadIpfs) {
        $args += "--upload-ipfs"
    }

    Write-Host "[baseline] running sample pipeline..."
...
if ($exitCode -ne 0) {
    Write-Error "[baseline] sample pipeline failed. ExitCode=$exitCode"
}
